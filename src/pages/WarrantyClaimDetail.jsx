...
const ClaimDetail = () => {
  ...
  const handleAddPart = () => {
    ...
    if (existsIndex >= 0) {
      ...
      setEditedParts([
        ...editedParts,
        { ...selectedPart, category: selectedCategory, quantity: partQuantity },
      ]);
    }
    ...
    <div className="bg-gray-50 min-h-screen py-6 px-4 sm:px-6 lg:px-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-sm text-gray-500 mb-2">
          ...
          <span className="mx-1">/</span>
          <Link className="text-gray-700 font-medium">Claim Detail</Link>
        </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <InfoItem
              label="Recall Status"
              value={
                fcr?.statusRecall === "AGREED_RECALL"
                  ? "AGREED_RECALL"
                  : fcr?.statusRecall || "No Recall"
              }
            />
          </div>
                ...
                editedParts.some(
                  (p) =>
                    ...
                    (p.recommendedQuantity &&
                      p.recommendedQuantity > p.remainingStock)
                ) &&
                (() => {
                  const hasNoStock = editedParts.some(
                    (p) => p.remainingStock === 0
                  );
                  const hasLowStock = editedParts.some(
            ...
            <div className="flex justify-end gap-3 mt-5">
              ...
              <button
                onClick={async () => {
                  ...
                  const invalidParts = tempParts.filter(
                    (p) => p.quantity > (p.remainingStock ?? Infinity)
                  );
                  if (invalidParts.length > 0) {
                    toast.error(
                      `Quantity for ${invalidParts
                        .map((p) => p.name)
                        .join(", ")} exceeds available stock`
                    );
                  ...
                  const invalidRecommend = tempParts.filter(
                    (p) =>
                      p.recommendedQuantity &&
                      p.quantity > p.recommendedQuantity
                  );
                  if (invalidRecommend.length > 0) {
                    toast.error(
                      `Quantity for ${invalidRecommend
                        .map((p) => p.name)
                        .join(", ")} is below recommended level`
                    );
                    return;
                  }
